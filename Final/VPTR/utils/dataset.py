...
def get_dataloader(data_set_name, batch_size, data_set_dir, test_past_frames = 10, test_future_frames = 10, ngpus = 1, num_workers = 1):
    ...
    elif data_set_name == 'MASK':
        renorm_transform = VidToTensor()
        train_transform = VidToTensor()
        test_transform = VidToTensor()

        dataset_dir = Path(data_set_dir)
        train_set = MaskDataset(("AA"), train_transform)
        val_set = MaskDataset(("AA"), train_transform)
        test_set = MaskDataset(("AA"), train_transform)
...
class MaskDataset(Dataset):
    """
    Dataset that load data from all mask.npy files
    """
    def __init__(self, data_path, transform):
        """
        both num_past_frames and num_future_frames are limited to be 10
    ...
    def load_data(self):
        ...
        cnt = all_videos.count
        data = np.array(cnt,22,9,160,240)
        for i in all_videos:
    ...
    def __getitem__(self, index: int):
        ...
        clips = self.data[index, :, :, :, :]

        past_clip = clips[:, 0:11, :, :, :]
        future_clip = clips[:, 11:22, :, :, :]
